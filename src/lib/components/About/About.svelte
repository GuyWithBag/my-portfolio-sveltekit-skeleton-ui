<script lang="ts">
	...
	onMount(() => {
		onObserve(document.querySelector('#about'), {
...
<div class="h-screen">
	<div id="about" class="blue-feel flex h-screen items-center justify-center relative z-[1]">
		{#if visible}
			<div class="grid lg:grid-cols-2 w-[60rem] justify-center items-center">
				<div transition:spin={{ duration: 1000, easing: cubicInOut, times: 8 }}>
					<img src={LuluGIF} alt="Lulu Legends Of Runterra Emote" />
				</div>
				<div class="grid justify-items-start items-start justify-center gap-2">
					<div transition:fly={{ x: 200, duration: 1000, easing: cubicInOut }}>
						<AboutParagraphBlock>
							I am a young passionate learner who has much more to offer in the future with my
							creative ideas.
						</AboutParagraphBlock>
					</div>
					<div
						in:fly={{ y: 200, duration: 700, delay: 500, easing: cubicInOut }}
