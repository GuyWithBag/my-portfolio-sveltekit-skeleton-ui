<script lang="ts">
	...
	function downloadResume() {
		download(
			'src/lib/assets/downloads/LoejeeMiguel_Dulaugon_Resume.pdf',
			'LoejeeMiguel_Dulaugon_Resume.pdf'
		);
	}

	onMount(() => {
		onObserve(document.querySelector('#title-card'), {
			isIntersecting: () => {
...
<div id="title-card" class="h-screen flex items-center justify-center relative z-[0]">
	{#if titleCardVisiblity == true}
		...
		<div
			in:fade={{ duration: spaceBgFade, easing: quadIn }}
			out:fade={{ duration: 0.5, easing: quadIn }}
			class=" space-background h-full w-full flex items-center justify-center relative z-[0]"
		>
			<div
				class="relative z-[3]"
				style:transform={`translate3d(0, ${scrollY * parallaxSpeed}px, 0)`}
			>
				<div class="h-screen flex items-center justify-center">
					<div class="grid lg:grid-cols-2 gap-x-8 ga-y-1 place-items-center lg:w-[50rem] w-[19rem]">
						<div class="flex flex-col lg:gap-y-5 gap-1">
							...
							<p
								class="lg:text-[15px] lg:text-left text-center"
								transition:fly={{ x: -250, duration: generalDuration, delay: spaceBgFade + 500 }}
							>
								I am a flexible programmer learner capable of learning and adapating very fast, game
							...
							<div class="grid grid-cols-3 gap-1 lg:gap-3">
								<a
									href="/downloads/LoejeeMiguel_Dulaugon_Resume.pdf"
